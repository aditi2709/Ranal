 ...
 */
 ...
 class Geogo_Projects_Wide_Section extends WP_Widget {
   ...
   function __construct() {
     ...
     $widget_ops = array(
       'classname'                   => 'geogo_projects_wide_section',
       'description'                 => esc_html__( 'Displays a section with geogo projects in wide two column layout', 'shapely-companion-geogo' ),
       'customize_selective_refresh' => true,
     );

     parent::__construct( 'geogo_projects_wide_section', esc_html__( '[Geogo] Projects Wide Section', 'shapely-companion-geogo' ), $widget_ops );
   ...
   */
   // display widget
    ...
    ?>
     ...
     <div class="widget pt50" id="geogo-projects-widget">
       <div class="container">
         <div class="row">
           <div class="col-md-7 col-sm-7 mb-xs-2">
             <span class="go-widget-blue-text"><?php echo $sec_heading ?></span>
             <?php if ( $title ) { ?>
               <h3 class="go-section-title"><?php echo $title ?></h3>
             <?php } ?>
             <?php if ( '' != $body_content ) { ?>
               <div class="mb32 go-subtitle mb-xs-2"><?php echo $body_content ?></div>
             <?php } ?>
           </div>
         </div>
       </div>
       <div class="custom-navigation">
          <a class="btn-icon-round muted flex-prev" href="#"><i class="fa fa-long-arrow-left"></i></a>
          <div class="custom-controls-container"></div>
          <a class="btn-icon-round flex-next" href="#"><i class="fa fa-long-arrow-right"></i></a>
         ...
         <div class="flexslider">
           <ul class="slides">
             <?php
               ...
               $total_posts = sizeof($latest_posts);
               foreach($latest_posts as $key=>$post ) {
                 $title = isset( $post->post_title ) ? $post->post_title : '';
                 $current_post_serial = ($key + 1);
                 $short_description = get_post_meta( $post->ID, 'short_description', true );;
                 $feedback = get_post_meta( $post->ID, 'feedback', true );;
                 $feedback_from = get_post_meta( $post->ID, 'feedback_from', true );;
             ?>
            <li>
              <div class="row">
                <div class="col-lg-6 pr0 hidden-sm hidden-xs">
                ...
                <div class="col-lg-6 pl0">
                  <div class="flex-caption p-xs-30-40">
                    <h6 class="slide-count">
                      <?php echo "{$current_post_serial} / {$total_posts}" ?>
                    </h6>
                    <h2><?php echo $title ?></h2>
                    <p class="go-card-text"><?php echo $short_description ?></p>
                    <blockquote class="feedback-text"><?php echo $feedback ?></blockquote>
                    <p class="feedback-from"><?php echo $feedback_from ?></p>
                    <br />
                    <a class="btn btn-lg btn-filled" href="<?php echo esc_url( get_permalink($post->ID) ); ?>">Read Full Story<i class="fa fa-angle-double-right btn-icon"></i></a>
                  </div>
                </div>
              </div>
            </li>
  ...
  // register widget
  add_action('widgets_init', create_function('', 'return register_widget("Geogo_Projects_Wide_Section");'));
?>
