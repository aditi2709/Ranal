...
class Shapely_Home_Call_For_Action extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		<?php if ( '' != $instance['title'] ) : ?>
			<section class="cfa-section bg-secondary <?php echo wp_kses_post( $instance['css_classes'] ) ?>">
				<div class="container">
					<div class="row">
						<div class="col-sm-12 text-center p0">
							<div class="overflow-hidden">
								...
								<div class="col-sm-3">
									<a href="<?php echo esc_url_raw( $instance['button_link'] ); ?>" class="mb0 btn btn-lg cfa-button"><?php echo esc_html( $instance['button'] ); ?></a>
								</div>
							</div>
						</div>
					</div>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'css_classes' ) ); ?>">
				<?php echo esc_html__( 'Additional CSS classes ', 'shapely-companion' ); ?>
			</label>
			<input type="text" value="<?php echo $instance['css_classes'] ; ?>" name="<?php echo esc_attr( $this->get_field_name( 'css_classes' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'css_classes' ) ); ?>" class="widefat" />
		</p>
		<?php
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['css_classes']       = ( ! empty( $new_instance['css_classes'] ) ) ? wp_kses_post( $new_instance['css_classes'] ) : '';
