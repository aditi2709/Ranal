...
function shapely_search_form( $form ) {
    ...
    <input class="rounded-text-box" type="text" placeholder="' . esc_html__( 'Type Here', 'shapely' ) . '" value="' . esc_attr( get_search_query() ) . '" name="s" id="s" />
    <button style="border-radius: 30px !important;" type="submit" class="searchsubmit"><i class="fa fa-search" aria-hidden="true">  search</i><span class="screen-reader-text">' . esc_attr__( 'Search', 'shapely' ) . '</span></button>
    </form>';
						...
						<div class="<?php echo $breadcrumbs_enabled ? 'col-md-6 col-sm-6 col-xs-12' : 'col-xs-12'; ?>">
							<h3 class="page-title" <?php echo $header_color ? 'style="color:#' . esc_attr( $header_color ) . '"' : ''; ?>>
								...
								if ( is_home() ) {
									...
									}elseif(get_queried_object()->taxonomy=='ranal_solutions'){// Custom Condition for Custom Posttype
